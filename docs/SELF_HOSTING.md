...
OkuGym is two small containers (a web server and an API) plus a folder of your data.
...
OkuGym signs you in with **passkeys** (WebAuthn). Browsers enforce two rules:
...
passkey prompt won't appear. To use OkuGym from your phone you need a real HTTPS hostname.
...
Put OkuGym behind something that terminates TLS for a hostname you control, then point it at
...
OkuGym only needs the browser to reach it over `https://gym.example.com`.
...
```bash
tar czf okugym-backup-$(date +%F).tar.gz data/
```
...
OkuGym can push two kinds of alert to your phone/desktop, even when the app isn't open:
