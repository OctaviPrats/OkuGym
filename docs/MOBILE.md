...
OkuGym ships in two flavors from the same codebase:
...
State is mirrored from `localStorage` into `okugym-state.json` in the app's private data
...
- **iOS:** a Mac with Xcode 15+ and CocoaPods (`brew install cocoapods`). A free Apple ID
  ...
  for App Store distribution, which OkuGym doesn't do.
...
OkuGym's mobile app is not on the Play Store or App Store, and that's a choice: no store
accounts, no store rules, no yearly fees between you and an open-source app.

### Android — sideload the APK

No official OkuGym APK release is published from this repository yet. Use this guide
to build a local APK yourself.
...
keytool -genkeypair -keystore my.keystore -alias okugym -keyalg RSA -validity 10950

# align + sign (zipalign/apksigner ship with the Android SDK build-tools)
zipalign -f -p 4 app-release-unsigned.apk aligned.apk
apksigner sign --ks my.keystore --ks-key-alias okugym --out OkuGym.apk aligned.apk
```
