...
OkuGym is also a standalone mobile app — and it ships as a direct APK download, not
through app stores.

- 📱 **Standalone mobile app.** The same frontend now also builds as a native iPhone /
  Android app (Capacitor) — the install-and-done flavor of OkuGym: no account, no server,
...
- 🤖 **Android APK, no Play Store.** The official build is a signed, sideloadable APK
  (~4.5 MB) from [okugym.duarte-santos.ch](https://okugym.duarte-santos.ch) — deliberately
...
- ▶️ **Live demo** at [duartesantos8.github.io/OkuGym](https://duartesantos8.github.io/OkuGym/) —
...
- 🐳 Prebuilt Docker images published to `ghcr.io/duartesantos8/okugym-{api,web}` (amd64 + arm64)
