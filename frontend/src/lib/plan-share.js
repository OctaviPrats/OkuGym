...
//
// Two jobs:
//  1. A small, self-contained file a friend can import into THEIR OkuGym — just the
...
export function buildPlanBundle(S, name) {
  ...
  return { okugym_plan: PLAN_FMT, exported: todayISO(), name: name || '', week, routines, customEx }
}
...
export function parsePlan(raw) {
  ...
  if (!data || !data.okugym_plan || !Array.isArray(data.routines)) {
    throw new Error(t('this isn’t an OkuGym plan file'))
  }
...
<body><div class="doc">
  ...
  ${body}
  <footer>${esc(t('Made with OkuGym'))} · github.com/OctaviPrats/OkuGym</footer>
</div></body></html>`
}
