...
OkuGym is a self-hosted app: you run the server, you hold the data. This file says which
...
patched. A fix ships in the next release, and self-hosters should update to the latest source.
...
<https://github.com/OctaviPrats/OkuGym/security/advisories/new>
...
Read this before hosting OkuGym for anyone other than yourself.
...
- **Nothing in `./data` is encrypted.** It holds `db.json` (users, passkey public keys, push
  ...
  `secret` can mint a valid session cookie for any account. **If you host OkuGym for other
